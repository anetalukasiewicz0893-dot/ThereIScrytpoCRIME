...
export const CryptoQuotes: React.FC = () => {
  ...
  const fetchQuote = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: 'Provide a very short, powerful, and insightful crypto quote (e.g., from Satoshi Nakamoto, Vitalik Buterin, or general crypto culture wisdom). Keep it impactful, professional, and under 15 words.',
      });
      setQuote(response.text?.replace(/"/g, '') || 'The root problem with conventional currency is all the trust that’s required to make it work.');
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto mt-20 p-16 bg-slate-900/30 border border-slate-800 rounded-[3rem] text-center space-y-10 backdrop-blur-3xl shadow-2xl animate-in zoom-in-95 duration-500">
      ...
      <h3 className="text-xs font-black text-cyan-500 uppercase tracking-[0.4em]">Crypto Quote of the Day</h3>
