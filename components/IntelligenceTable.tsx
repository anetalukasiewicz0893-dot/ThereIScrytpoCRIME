  ...
  if (data.length === 0) {
    return (
      <div className="bg-slate-900/10 border-2 border-dashed border-slate-800/40 rounded-[3rem] p-40 text-center">
        <p className="text-slate-500 font-black text-xl tracking-widest uppercase italic">Vault Empty. Everyone is either safu or too smart to get caught.</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-950/80 text-slate-500 text-[10px] uppercase font-black tracking-[0.3em] border-b border-slate-900 print:text-black print:bg-slate-100">
              ...
              <th className="px-6 py-8">Funds Lost</th>
              <th className="px-6 py-8">Alpha Grounding</th>
          ...
          <tbody className="divide-y divide-slate-800/40 print:divide-slate-200">
            {data.map((item) => {
              ...
              return (
                <tr 
                  key={item.id} 
                  className={`group transition-colors ${isHighPriority ? 'bg-rose-900/10 border-l-4 border-l-rose-600 print:bg-rose-50' : 'hover:bg-white/[0.01]'}`}
                >
                  ...
                  <td className="px-6 py-10">
                     <div className={`text-[13px] font-mono font-black tracking-tight ${isHighValue ? 'text-rose-500' : 'text-white print:text-black'}`}>
                       {item.amount}
                     </div>
                     <div className={`text-[8px] mt-2 font-black uppercase tracking-widest px-2 py-0.5 inline-block rounded ${isHighPriority ? 'text-rose-500 bg-rose-500/10' : 'text-slate-600 bg-slate-800 print:bg-slate-200'}`}>
                        {item.priority}
                     </div>
                  </td>
                  <td className="px-6 py-10 max-w-lg">
                    ...
                    <div className="mt-5 flex gap-6 print:hidden">
                      <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="text-[9px] font-black uppercase text-cyan-600 hover:text-cyan-400 transition-all border-b border-cyan-900 flex items-center gap-2">Source Link</a>
                    </div>
                  </td>
                  <td className="px-10 py-10 print:hidden">
                    <div className="flex flex-col gap-3">
                      <button onClick={() => onSave(item.id)} className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] border transition-all ${item.isSaved ? 'bg-cyan-500 text-black border-cyan-500' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-white'}`}>
                        {item.isSaved ? 'Pinned' : 'Pin Intel'}
                      </button>
                      <button onClick={() => onDiscard(item.id)} className="text-[8px] font-black uppercase text-rose-900 hover:text-rose-500 text-left px-2 underline decoration-rose-900/30">Discard Alpha</button>
                    </div>
                  </td>
                </tr>
              );
